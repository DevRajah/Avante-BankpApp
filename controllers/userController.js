...
const { validateUser, validateUserLogin, validateResetPassword, validateUserPersonalProfile } = require('../middlewares/validator');
...
const signUp = async (req, res) => {
  try {
    ...
    if (error) {
      ...
      if (userData.tel) {
        const checkTel = await userModel.findOne({ tel: userData.tel })
        if (checkTel) {
          return res.status(200).json({
            message: 'Telphone number already exists!',
          });
        }
      }

      if (userData.company) {
        const companyExists = await userModel.findOne({ company: userData.company });
        if (companyExists) {
          return res.status(200).json({
      ...
      return res.status(200).json({
        message: 'User profile created successfully!, Please check your email to verify your account',
        data: {
...
const loginWithEmail = async (req, res) => {
  try {
    ...
    const token = jwt.sign({
      userId: user._id,
      email: user.email,
    }, process.env.SECRET, { expiresIn: "1day" });
...
const resetPassword = async (req, res) => {
  try {
    ...
    const updatedUser = await userModel.findByIdAndUpdate(userId, { password: hashPassword }, { new: true });
...
const updatePersonalProfile = async (req, res) => {
  try {
    ...
    const profileData = {
      ...
      tel: req.body.tel || profile.tel,
      gender: req.body.gender || profile.gender,
      companyName: req.body.companyName || profile.companyName,
      age: req.body.age || profile.age,
      mainProducts: req.body.mainProducts || profile.mainProducts,
    };

    // Update the user profile
    const newProfile = await userModel.updateOne({ _id: userId }, profileData, { new: true });
    if (!newProfile) {
      return res.status(404).json({
...
const updateContactInformation = async (req, res) => {
  try {
    ...
    const profileData = {
      ...
      alternativeEmail: req.body.alternativeEmail || profile.alternativeEmail,
...
const updateCompanyInformation = async (req, res) => {
  try {
    ...
    const profileData = {
      companyName: req.body.companyName || profile.companyName,
      businessType: req.body.businessType || profile.businessType,
      officialWebsite: req.body.officialWebsite || profile.officialWebsite,
      registeredAddress: req.body.registeredAddress || profile.registeredAddress,
      operationalAddress: req.body.operationalAddress || profile.operationalAddress,
      taxInformation: req.body.taxInformation || profile.taxInformation,
      platformForSelling: req.body.platformForSelling || profile.platformForSelling,
      yearEstablished: req.body.yearEstablished || profile.yearEStablished,
      totalNumberOfEmployees: req.body.totalNumberOfEmployees || profile.totalNumberOfEmployees,
